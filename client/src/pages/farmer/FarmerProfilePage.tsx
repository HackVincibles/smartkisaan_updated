...
const FarmerProfilePage = () => {
  ...
  const stats = [
    ...
    { label: 'Verified', value: user.isVerified ? 'Yes' : 'No', icon: ShieldCheck, color: 'text-green-600', bg: 'bg-green-50' },
    { label: 'KYC Status', value: user.isKYCDone ? 'Complete' : 'Pending', icon: User, color: 'text-blue-600', bg: 'bg-blue-50' },
  ];
    ...
    <DashboardLayout>
      <div className="max-w-4xl mx-auto space-y-6">
        ...
        <div className="card bg-white p-8 overflow-hidden relative">
          <div className="absolute top-0 left-0 w-full h-32 bg-gradient-to-r from-primary-500 to-primary-700" />
            ...
            <div className="relative">
              <div className="w-32 h-32 rounded-3xl bg-white p-1 shadow-xl">
                {user.profilePhoto ? (
                  ...
                  <div className="w-full h-full rounded-[1.25rem] bg-gray-100 flex items-center justify-center text-primary-600">
                    <User size={48} />
                  </div>
                )}
              </div>
          ...
          <div className="card bg-white p-6 space-y-6">
            ...
            <div className="space-y-4">
              <div className="p-4 rounded-2xl bg-gray-50 border border-gray-100">
                ...
                <p className="font-bold text-gray-900">{user.upiId || 'Not set'}</p>
              </div>

              <div className="p-4 rounded-2xl bg-gray-50 border border-gray-100 space-y-3">
                <div>
                  <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">Bank Account</p>
                  <p className="font-bold text-gray-900">{user.bankAccount?.accountNumber ? `•••• •••• ${user.bankAccount.accountNumber.slice(-4)}` : 'Not set'}</p>
                </div>
                {user.bankAccount?.ifscCode && (
                  <div>
                    <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">IFSC Code</p>
                    <p className="font-bold text-gray-900">{user.bankAccount.ifscCode}</p>
                  </div>
                )}
              </div>
            </div>
          ...
          <button className="w-full flex items-center justify-between p-4 hover:bg-gray-50 rounded-2xl transition-colors group">
            <div className="flex items-center gap-4">
              ...
              <div className="text-left">
                ...
                <p className="text-xs text-gray-500">Update password and authentication methods</p>
              </div>
            </div>
